...
// Object objects[] = {
    ...
    new Sphere(8, Vec(34, 9+.0001, 166),        Vec(),         Vec(1,.5,.5),REFR),
    new Sphere(8, Vec(145, 9+.0001, 84),        Vec(),         Vec(.8,.4,.8),REFR),
    new Sphere(14, Vec(111, 15+.0001, 132),        Vec(),         Vec(1,1,1)*.999,DIFF,"cueball.png"),
    new Sphere(25, Vec(86, 26+.0001, 97),        Vec(),         Vec(1,1,1)*.999,DIFF,"ball.jpg"),
...
inline bool intersect(const Ray &R, double &t, int &id, unsigned short* X, bool debug = false) {
    double d, inf = t = 1e20;
    for(int i = objects_num; i--;) {
        if (debug) puts("intersect.in"), Bezier_debug = true;
        d = objects[i]->intersect(R, X);
